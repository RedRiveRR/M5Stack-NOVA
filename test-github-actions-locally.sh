...
echo "Testing M5Stick-Nova compilation with GitHub Actions dependencies"
echo
...
EXTRA_FLAGS="$EXTRA_FLAGS -DNova_VERSION=\"$VERSION\" -D$LANGUAGE"
...
# Show build results
if [ -f "build/${FQBN//:/.}/m5stick-Nova.ino.bin" ]; then
    echo "Firmware binary: build/${FQBN//:/.}/m5stick-Nova.ino.bin"
    ls -la "build/${FQBN//:/.}"/m5stick-Nova.ino.*
fi
