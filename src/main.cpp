...
// Settings handled by PlatformIO build flags now
// #define STICK_C_PLUS
...
// -=-=- NOVA Language for Menu and Portal -=-=-
// Languages are now selectable in settings
...
uint8_t read_bits(uint8_t bits);

// Default language index (will be loaded from EEPROM)
...
#include <esp_bt.h>
  ...
  //#define BITMAP M5.Display.drawBitmap(0, 0, 320, 240, NovaMatrix) // This doesn't work, generates static.
...
// 18 - QR Codes
// 19 - NOVA Portal (with Clone support)
...
int lanmenu_size = sizeof(lanmenu) / sizeof(MENU);

void lanmenu_onSelect() {
  selected_language = lanmenu[cursor].command;
  #if defined(USE_EEPROM)
    EEPROM.write(11, selected_language);
    EEPROM.commit();
  #endif
  // Reboot to apply changes since localized text is used in many static initializers
  DISP.fillScreen(BGCOLOR);
  DISP.setCursor(0, 40);
  DISP.println("Applying Language...");
  DISP.println("Yeniden Baslatiliyor...");
  delay(2000);
  ESP.restart();
}

void lanmenu_setup() {
  menuController.setup(lanmenu, lanmenu_size, nullptr, lanmenu_onSelect);
}
...
Serial.begin(115200);
  ...
  if(check_next_press()){
    ...
    Serial.printf("EEPROM 11 - Language:   %d\n", EEPROM.read(11));
    
    if(EEPROM.read(0) > 3 || EEPROM.read(1) > 240 || EEPROM.read(2) > 100 || EEPROM.read(3) > 1 || EEPROM.read(4) > 19 || EEPROM.read(5) > 19 || EEPROM.read(6) > 100 || EEPROM.read(8) > 100 || EEPROM.read(9) > 100 || EEPROM.read(10) > 100 || EEPROM.read(11) > 4 ) {
      ...
      EEPROM.write(11, 0);   // Default language: English (0)
      EEPROM.commit();
    }
    ...
    ph_alert_ssids = EEPROM.read(10);
    selected_language = EEPROM.read(11);
    if (selected_language > 4) selected_language = 0; // Final safety clamp
    rebuildMenus(); // Populate menu strings based on loaded language
  #endif
  getSSID();
...
ProcessHandler processes[] = {
  ...
  {20, lanmenu_setup, menu_controller_loop, "Language Menu"},
...
void rebuildMenus() {
  ...
  int i = 0;
  #if defined(RTC)
    mmenu[i++].name = TXT_CLOCK;
  #endif
  mmenu[i++].name = TXT_MN_TV;
  mmenu[i++].name = TXT_MN_BT;
  mmenu[i++].name = TXT_MN_WF;
  #if defined(CARDPUTER)
  ...
  i = 0;
  smenu[i++].name = TXT_BACK;
  #if defined(AXP) || defined(PWRMGMT)
    smenu[i++].name = TXT_BATT_INFO;
  #endif
  #if defined(CARDPUTER)
    smenu[i++].name = TXT_BATT_INFO;
  #endif
  smenu[i++].name = TXT_BRIGHT;
  #if defined(RTC)
    smenu[i++].name = TXT_SET_CLOCK;
  #endif
  #if defined(ROTATION)
    smenu[i++].name = TXT_ROTATION;
  #endif
  #if defined(SDCARD)
    #ifndef CARDPUTER
      smenu[i++].name = TXT_SDCARD;
    #endif
  #endif
  i += 4; // Skip RSSI and Alert settings for now as they are name-safe
  smenu[i++].name = TXT_PH_ALERT_SSIDS;
  smenu[i++].name = "Language";
  smenu[i++].name = TXT_THEME;
  smenu[i++].name = TXT_ABOUT;
  smenu[i++].name = TXT_REBOOT;
  #if defined(USE_EEPROM)
    smenu[i++].name = TXT_CLR_SETTINGS;
  #endif

  // Dimmer Menu
  i = 0;
  dmenu[i++].name = TXT_BACK;
  dmenu[i++].name = TXT_NEVER;
  dmenu[i++].name = TXT_5SEC;
  dmenu[i++].name = TXT_10SEC;
  dmenu[i++].name = TXT_15SEC;
  dmenu[i++].name = TXT_30SEC;
  dmenu[i++].name = TXT_60SEC;
  dmenu[i++].name = TXT_120MIN;
  dmenu[i++].name = TXT_240MIN;

  // TV-B-Gone Menu
  i = 0;
  tvbgmenu[i++].name = TXT_BACK;
  tvbgmenu[i++].name = TXT_MN_AMERICA;
  tvbgmenu[i++].name = TXT_MN_EMEA;

  // WiFi Spam Menu
  i = 0;
  wsmenu[i++].name = TXT_BACK;
  wsmenu[i++].name = TXT_WF_SCAN;
  wsmenu[i++].name = TXT_WF_SPAM_FUN;
  wsmenu[i++].name = TXT_WF_SPAM_RR;
  wsmenu[i++].name = TXT_WF_SPAM_RND;
  wsmenu[i++].name = "Nova Portal";
  wsmenu[i++].name = "Deauth Hunter";
  wsmenu[i++].name = TXT_PINEAP_HUNTER;

  // Language Menu
  i = 0;
  lanmenu[i++].name = "English";
  lanmenu[i++].name = "Turkce";
  lanmenu[i++].name = "Italiano";
  lanmenu[i++].name = "Portugues";
  lanmenu[i++].name = "Francais";

  // QR Codes Menu
  qrcodes[0].name = TXT_QR_BACK;

  // Bluetooth Menu
  i = 0;
  btmenu[i++].name = TXT_BACK;
  btmenu[i++].name = "AppleJuice";
  btmenu[i++].name = "Swift Pair";
  btmenu[i++].name = "Android Spam";
  btmenu[i++].name = TXT_SA_CRASH;
  btmenu[i++].name = "BT Maelstrom";
  btmenu[i++].name = "BLE Hunter";

  // AppleJuice Menu
  i = 0;
  ajmenu[i++].name = TXT_BACK;
  i++; // Skip AirPods
  ajmenu[i++].name = TXT_AJ_TRANSF_NM;
  // Others are static or fixed later
  ajmenu[28].name = TXT_AJ_TV_COLOR;
  ajmenu[29].name = TXT_STP_NW_PH;

  // Process table rebranding (Manual fix for consistency)
  for(int p=0; processes[p].id != -1; p++) {
     if (processes[p].id == 1) processes[p].name = "Nova Menu";
     if (processes[p].id == 2) processes[p].name = "Nova Settings";
     if (processes[p].id == 19) processes[p].name = "Nova Portal";
  }
}
