...
arduino-cli compile --log \
  ...
  --output-dir ./m5stick-Nova/build \
  ./m5stick-Nova/m5stick-Nova.ino

if [[ "${locale}" == "en-us" ]]; then
  output_file="M5Nova-${name}.bin"
else
  output_file="M5Nova-${name}.${locale}.bin"
fi

echo "Merging binaries to ${output_file}"

esptool.py --chip esp32s3 merge_bin --output ./m5stick-Nova/build/${output_file}  \
            ${bootloader_addr} ./m5stick-Nova/build/m5stick-Nova.ino.bootloader.bin \
            0x8000 ./m5stick-Nova/build/m5stick-Nova.ino.partitions.bin \
            0x10000 ./m5stick-Nova/build/m5stick-Nova.ino.bin
